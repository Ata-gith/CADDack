...
# replace mol_to_ecfp_bits with:
def mol_to_ecfp_bits(m: Chem.Mol, radius: int = 2, n_bits: int = 2048) -> dict[str, int]:
    """
...
def smiles_to_features(smiles: str, radius: int = 2, n_bits: int = 2048) -> dict | None:
...
def featurize_dataframe(df: pd.DataFrame, smiles_col: str = "SMILES", radius: int = 2, n_bits: int = 2048) -> pd.DataFrame:
    out_rows = []
    for _, row in df.iterrows():
        smi = row.get(smiles_col)
        f = smiles_to_features(smi, radius=radius, n_bits=n_bits)
        if f is None:
            out_rows.append({**row.to_dict(), "__error": "invalid_smiles"})
        else:
            out_rows.append({**row.to_dict(), **f})
